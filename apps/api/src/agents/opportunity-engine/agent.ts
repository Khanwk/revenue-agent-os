...
export const opportunityEngineAgent: AgentDefinition<EngineInput, EngineOutput> = {
  meta: {
    id: "opportunity-engine", name: "Opportunity Engine", shortName: "Scout", icon: "radar", version: "1.0.0",
  ...
  async execute(input, context) {
    context.report("tool", "Company profile", "Loading skills, services, budget thresholds and preferred work.");
    const profile = await getCompanyProfile();
    const queries = buildQueries(input, profile.skills, profile.preferredKeywords);

    context.report("tool", "Scanning sources", "Searching every configured project source.", queries.join(" · "));
    const searched = await searchAllSources(queries, Math.max(5, Math.ceil(env.SOURCE_FETCH_LIMIT / queries.length)));

    context.report("thinking", "Fast ranking", `Pre-scoring ${searched.opportunities.length} projects before spending AI tokens.`);
    ...
    context.report("thinking", "Deep fit analysis", `Analyzing the top ${aiCandidates.length} matches against delivery ability and commercial value.`);
    const mockResponse = {
    ...
    context.report("validating", "Ranking", "Combining deterministic pre-score with AI judgement and checking links/results.");
