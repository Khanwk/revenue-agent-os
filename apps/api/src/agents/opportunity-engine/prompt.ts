...
export function buildEnginePrompt(profile: CompanyProfile, jobs: Array<{ job: DiscoveredOpportunity; quickScore: number; matchedSkills: string[] }>) {
  const compactJobs = jobs.map(({ job, quickScore, matchedSkills }) => ({
    projectId: job.id, platform: job.platformLabel, title: job.title, description: job.description.slice(0, 1800), budget: job.budget,
    skills: job.skills, location: job.location, proposals: job.proposals, quickScore, matchedSkills
  }));
