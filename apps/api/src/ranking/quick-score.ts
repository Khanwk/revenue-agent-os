...
export function quickScore(
  ...
  const weightedTotal =
    profile.skills.reduce((sum, skill) => sum + skill.strength, 0) || 1;
  ...
  const skillScore = Math.min(
    45,
    Math.round((matchedWeight / Math.max(12, weightedTotal * 0.45)) * 45),
  );

  let budgetScore = 8;
  if (job.projectType === "fixed" && job.budgetMaxUsd !== undefined) {
    if (job.budgetMaxUsd >= profile.minimumFixedBudgetUsd * 4) budgetScore = 20;
    else if (job.budgetMaxUsd >= profile.minimumFixedBudgetUsd * 2)
      budgetScore = 16;
    else if (job.budgetMaxUsd >= profile.minimumFixedBudgetUsd)
      budgetScore = 12;
    else budgetScore = 2;
  ...
  const preferredMatches = profile.preferredKeywords.filter((term) =>
    includes(text, term),
  );
  ...
  const penalties = profile.avoidKeywords.filter((term) =>
    includes(text, term),
  );
  ...
  const reasons = [
    matchedSkills.length
      ...
      : "Few explicit skill matches",
    preferredMatches.length
      ? `Preferred work signals: ${preferredMatches.slice(0, 4).join(", ")}`
      : "No preferred-keyword bonus",
    job.proposalCount !== undefined
      ? `${job.proposalCount} competing bids/proposals reported`
