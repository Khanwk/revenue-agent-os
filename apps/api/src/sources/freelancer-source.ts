...
function money(project: AnyRecord) {
  const currency = project.currency?.code || project.currency?.sign || "USD.js";
...
function projectUrl(project: AnyRecord) {
  const seo = String(project.seo_url || "")
    .replace(/^\/+/, "")
    .replace(/^projects\//, "");
  return seo
...
export const freelancerOpportunitySource: OpportunitySource = {
  status: () => ({
    ...
    platform: "freelancer",
    configured: true,
    note: "Uses Freelancer.com's public active-project API.",
  }),
  async search(input) {
    ...
    const response = await fetch(
      `https://www.freelancer.com/api/projects/0.1/projects/active/?${params}`,
      {
        headers: {
          ...
          "User-Agent": "RevenueAgentOS/1.0",
        },
        signal: AbortSignal.timeout(15000),
      },
    );
    if (!response.ok)
      throw new Error(`Freelancer source returned HTTP ${response.status}`);
    ...
    return projects.map((project): DiscoveredOpportunity => {
      ...
      const isHourly = String(project.type || "")
        .toLowerCase()
        .includes("hourly");
      return {
        ...
        skills: Array.isArray(project.jobs)
          ? project.jobs
              .map((job: AnyRecord) => String(job.name || ""))
              .filter(Boolean)
          : [],
        clientInfo:
          [
            owner?.status?.payment_verified ? "Payment verified" : undefined,
            owner?.display_name || owner?.username,
          ]
            .filter(Boolean)
            .join(" · ") || "Public Freelancer client",
        url: projectUrl(project),
        postedAt: Number.isFinite(Number(project.submitdate))
          ? new Date(Number(project.submitdate) * 1000).toISOString()
          : undefined,
