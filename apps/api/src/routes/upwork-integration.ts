...
upworkIntegrationRouter.get("/status", (_req, res) => {
  res.json({ configured: oauthConfigured(), connected: hasStoredUpworkToken() });
});

upworkIntegrationRouter.get("/connect", (_req, res) => {
  if (!oauthConfigured()) {
    return res.status(400).json({ error: "Set UPWORK_CLIENT_ID, UPWORK_CLIENT_SECRET and UPWORK_REDIRECT_URI first." });
  }
...
upworkIntegrationRouter.get("/callback", async (req, res) => {
  try {
    ...
    if (!code || !validUntil || validUntil < Date.now()) throw new Error("Invalid or expired Upwork OAuth callback state.");
    ...
    if (!response.ok) throw new Error(`Upwork token exchange failed (HTTP ${response.status}).`);

    const token = (await response.json()) as { access_token: string; refresh_token?: string; expires_in?: number };
    await saveUpworkToken(token);
    return res.redirect(`${env.WEB_ORIGIN}/?upwork=connected`);
  } catch (error) {
    const message = encodeURIComponent(error instanceof Error ? error.message : "Upwork OAuth failed");
